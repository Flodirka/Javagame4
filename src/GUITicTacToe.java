    ...
    public void handleHumanTurn(int number) {
        if (TicTacToe.isCellFree(number)) {
            ...
            if (TicTacToe.getWinner() == TicTacToe.HUMAN) {
                JOptionPane.showMessageDialog(this, "Игра окончена: пользователь выигрывает!");
                System.exit(0);
            }
            if (TicTacToe.isDraw()) {
                JOptionPane.showMessageDialog(this, "Игра окончена: ничья!");
                System.exit(0);
            }
            TicTacToe.makeComputerTurn();
            printGameTable();
            if (TicTacToe.getWinner() == TicTacToe.COMPUTER) {
                JOptionPane.showMessageDialog(this, "Игра окончена: компьютер выигрывает!");
                System.exit(0);
            }
            if (TicTacToe.isDraw()) {
                JOptionPane.showMessageDialog(this, "Игра окончена: ничья!");
            ...
            JOptionPane.showMessageDialog(this, "Эта клетка уже занята! Введите другое число");
        }
    }
