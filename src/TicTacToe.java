...
public class TicTacToe {
    ...
    public static boolean handleHumanTurn(int number) {
        ...
        if (getWinner() == COMPUTER) {
            System.out.println("Игра окончена: компьютер выигрывает!");
            printGameTable();
            return false;
        }
        if (isDraw()) {
    ...
    public static int readHumanTurn() {
        while (true) {
            ...
            if (input.length() != 1 || input.charAt(0) < '1' || input.charAt(0) > '9') {
                System.out.println("Неверный ввод: введите любой номер из представленных 1,2,3,4,5,6,7,8,9");
            } else {
                int number = Integer.parseInt(input);
                if (isCellFree(number)) {
                    return number;
                } else {
                    System.out.println("Эта клетка уже занята! Введите другое число");
                }
            }
        }
    }
